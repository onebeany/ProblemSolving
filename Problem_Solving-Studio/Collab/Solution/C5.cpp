...
void checkAndUpdate(int x, int y, int curr_value, vector<vector<int>>& cells,
                    vector<vector<bool>>& visit, queue<pair<int, int>>& q, int& min_altitude);
...
int findDifficulty(vector<vector<int>>& cells, vector<vector<bool>>& visit, pair<int, int>& curr_cell){
    ...
    while(!q.empty()){
        ...
        checkAndUpdate(x, y-1, curr_value, cells, visit, q, min_altitude);  // 12시
        checkAndUpdate(x+1, y, curr_value, cells, visit, q, min_altitude);  // 3시
        checkAndUpdate(x, y+1, curr_value, cells, visit, q, min_altitude);  // 6시
        checkAndUpdate(x-1, y, curr_value, cells, visit, q, min_altitude);  // 9시
    }
